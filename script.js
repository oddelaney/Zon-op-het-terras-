...
function zonTijdInfo(lat, lon) {
  ...
  const isZon = altitude > 0;
  let gevoel = Math.round(10 + (altitude * 60)); // Simulatie gevoelstemperatuur

  if (isZon) {
    const sunset = times.sunset;
    const minutes = Math.max(0, Math.round((sunset - now) / 60000));
    return `Zon nog ± ${minutes} min.<br>Gevoelstemperatuur: ${gevoel}°C`;
  } else {
    const nextRise = times.sunrise;
    return `Zon terug rond ${nextRise.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}<br>Gevoelstemperatuur: ${gevoel}°C`;
  }
}
...
function updateMarkers(terrassen) {
  ...
  document.getElementById('status').textContent = `${zichtbaar} terrassen zichtbaar (${filter})`;
