...
function updateMarkers(terrassen) {
  ...
  if (terrassen.length === 0) {
    melding.textContent = "Geen terrassen gevonden in dit gebied.";
...
function laadTerrassenRond(lat, lon) {
  const melding = document.getElementById('melding');
  melding.textContent = "Even geduld… terrassen laden...";
  melding.style.display = "block";
  ...
  .catch(err => {
    melding.textContent = "Fout bij laden van terrasdata.";
    console.error(err);
  });
}
...
function initApp() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      const melding = document.getElementById('melding');
      melding.textContent = "Locatie niet beschikbaar. Standaardlocatie gebruikt.";
      melding.style.display = "block";
      laadTerrassenRond(52.1, 5.1);
    });
  } else {
    const melding = document.getElementById('melding');
    melding.textContent = "Geolocatie wordt niet ondersteund.";
    melding.style.display = "block";
    laadTerrassenRond(52.1, 5.1);
  }
}
