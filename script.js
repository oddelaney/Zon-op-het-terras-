...
function laadTerrassen(userLat, userLon) {
...
const filtered = terrassen.filter(t => afstandKm(t.lat, t.lon, userLat, userLon) <= 10);
toonMarkers(filtered, filter);
...
navigator.geolocation.getCurrentPosition(pos => {
  ...
  window.userCoords = [userLat, userLon];
  laadTerrassen(userLat, userLon);
}, () => {
  window.userCoords = [52.0907, 5.1214];
  laadTerrassen(52.0907, 5.1214); // fallback Utrecht
});
