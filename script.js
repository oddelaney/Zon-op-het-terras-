...
function zonTijdInfo(lat, lon, gevoel = 'ɡC') {
  const now = new Date();
...
fetch(`https://api.open-meteo.com/v1/forecast?latitude=${t.lat}&longitude=${t.lon}&current=apparent_temperature&timezone=auto`)
  .then(res => res.json())
  .then(data => {
    const gevoel = data.current.apparent_temperature;
    const info = zonTijdInfo(t.lat, t.lon, gevoel);
    marker.setPopupContent(`<b>${t.name}</b><br>${zonnig ? "Zonnetje!" : "Schaduw"}<br><i>${info}</i>${popupKnoppen(t.name)}`);
  });
