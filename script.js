...
document.addEventListener("DOMContentLoaded", function () {
  ...
  }, signal: controller.signal }).addTo(map);

  setTimeout(() => map.invalidateSize(), 500);

  // Plak hier de rest van de bestaande script_js_v733 functionaliteit (zonder kaartinit)
 = L.map('map').setView([52.1, 5.1], 8);
let markers = [];
let userMarker = null;
let lastKnownPos = null;

L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png'{z}/{x}/{y}{r}.png', {
  maxZoom: 19,
...
function zonTijdInfo(lat, lon) {
  ...
  if (altitude > 0) {
    ...
    const zonTijd = sunset.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}, signal: controller.signal });
    ...
    const zonTijd = sunrise.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}, signal: controller.signal });
...
function updateMarkers(terrassen) {
  ...
  terrassen.forEach(t => {
    ...
    if (filter === 'alle' || (filter === 'zon' && zonnig) || (filter === 'schaduw' && !zonnig)) {
      zichtbaar++;
      const marker = L.marker([t.lat, t.lon], { icon: zonnig ? zonnigIcon : schaduwIcon }, signal: controller.signal })
        .addTo(map)
  ...
  document.getElementById("status").innerText = `${zichtbaar} terrassen zichtbaar (${filter}, signal: controller.signal })`;
}
...
function laadTerrassenInZicht() {
  ...
  const query = `
    [out:json];
    (
      node["amenity"~"cafe|bar|pub|restaurant"](${b.getSouth()},${b.getWest()},${b.getNorth()},${b.getEast()}, signal: controller.signal });
      way["amenity"~"cafe|bar|pub|restaurant"](${b.getSouth()},${b.getWest()},${b.getNorth()},${b.getEast()}, signal: controller.signal });
    );
    out center;
  `;
  ...
  .then(data => {
    ...
    }, signal: controller.signal }).filter(t => t.lat && t.lon);
...
navigator.geolocation.getCurrentPosition(pos => {
  ...
  userMarker = L.circleMarker(lastKnownPos, {
    ...
    weight: 2,
    fillOpacity: 1
  }, signal: controller.signal }).addTo(map).bindPopup("Jij bent hier");
}, signal: controller.signal });
...
function deelTerras(lat, lon, name) {
  ...
  if (navigator.share) {
    navigator.share({ title: name, text: msg, url: link }, signal: controller.signal });
  } else {
